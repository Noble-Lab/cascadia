...
class AsfParser(BaseParser):
    ...
    def read(self) -> BaseParser:
        ...
        """
        n_skipped = 0
        with open(self.path) as asf:
            for line in tqdm(asf, desc=str(self.path), unit="lines"):
                line = line.split()
                if line[0] == 'BEGIN':
                    mzs = []
                    intensities = []
                    ...
                    precursor_charge = float(line[0][7])
                elif line[0][:4] == 'SCAN':
                    ...
                    rts.append(float(line[2]))
                    levels.append(float(line[3]))
                elif line[0] == 'END':
